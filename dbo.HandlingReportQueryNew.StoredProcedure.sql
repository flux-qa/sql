USE [Lumber]
GO
/****** Object:  StoredProcedure [dbo].[HandlingReportQueryNew]    Script Date: 3/18/2024 4:24:36 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[HandlingReportQueryNew]
...
L.CADTotalPieces, L.tripNumber, L.designerComments, L.tank as tank, I.internalDescription, I.oldcode, 
ISNULL(N.handleLocationAlpha, I.CADHandle) as CADHandle, 
I.CADHandle,
...
case when N.west4East0 IS NULL then '' when N.west4East0 = 4 then 'West' else 'East' end as targetHandleArea, 
U.unit as targetNumber, Z.minTarget, Z.maxTarget, U.UMStock as targetStock,
...
U.piecesStock, dbo.substituteUnit(S.unit) as substituteUnits,
case when N.west4East0 IS NULL then '' when N.west4East0 = 4 then 'West' else 'East' end as side
...
LEFT OUTER join (select distinct ps_OrderLines_RID, west4East0, rowNumber, handleLocationAlpha, designDate, drillNumber
from NewHandleOrders) as N on N.ps_OrderLines_RID = L.ID and N.designDate = CD.designDate and N.drillNumber = CD.drillNumber
...
order by ISNULL(N.west4East0,999), ISNULL(N.rowNumber,1), 
ISNULL(N.handleLocationAlpha, I.CADHandle), U.targetNumber, U.unit, S.unit, UL.length desc
GO
