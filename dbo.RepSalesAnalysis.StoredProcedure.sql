USE [Lumber]
GO
/****** Object:  StoredProcedure [dbo].[RepSalesAnalysis]    Script Date: 3/18/2024 4:24:36 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[RepSalesAnalysis] 

@fromDate   date = '01/01/2019',
@thruDate   date = '12/31/2019',
...
;
with w as (
    select left(C.fieldRep,2) as rep, count(*) as noLines,
    ...
    left outer join (select C.fieldRep, round(sum(case when tallyPct < 0 then 0 else tallyPct end),0) 
        as tallyPct
        from Customers C inner join Orders O on O.originalShipTo_RID = C.ID
        inner join OrderLines L on L.ob_Orders_RID = O.ID
        ...
        group by c.fieldRep) as Z on left(C.fieldRep,2)  = Z.fieldRep
    
    where L.dateShipped between @fromDate and @newThruDate
    and C.contractorFlag = 0 and (@customer = '%' or C.name like @customer)
    group by left(C.fieldRep,2)
 )
 select rep, noLines, byOthers, totSales, profit$, 
