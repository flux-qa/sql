USE [Lumber]
GO
/****** Object:  StoredProcedure [dbo].[ReadSubstituteUnits]    Script Date: 3/18/2024 4:24:36 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[ReadSubstituteUnits]
    
    @sourceUnit integer = 678551
    ...
    Z.unit, Z.unitTally, Z.ps_OrderLines_REN, Z.ps_OrderLines_RID,  Z.assignedTo
    ...
    inner join (select Unit, I.ID as itemNo, U.location, U.UMStock, U.dateReceived, 
        ...
        where U.UMstock > 0 and U.unitType = 'I' ) as Z 
            on I.ID = Z.itemNo and 
            (U.location = Z.location OR U.lastLocation = Z.location) and 
            U.dateReceived >= Z.dateReceived and PO.ob_Vendors_RID = Z.vendorNo
            and U.unit <> Z.unit and dbo.unitOriginalTallyToString(U.ID) = Z.unitTally
