USE [Lumber]
GO
/****** Object:  StoredProcedure [dbo].[JasperPaymentAnalysis]    Script Date: 3/18/2024 4:24:36 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[JasperPaymentAnalysis]
as  
...
select row_number() over (order by R.name) as ID, 1 as BASVERSION, getdate() as BASTIMESTAMP,
    ...
    case when Z.avgDivisor > 0 then round(Z.avgNumerator / Z.avgDivisor,0) else 0 end as avgDaysToPay,
    case when Z.paidOnTime + Z.paidLate + Z.numbNowLate = 0 then 0 else round(1.0 * z.paidLate /(Z.paidOnTime + Z.paidLate + Z.numbNowLate),2) end as paidLatePct,
    ...
    case when Z.paidLateWeighted = 0 then 0 else round(Z.totDaysLateWeighted / Z.paidLateWeighted,0) end as avgLateWeighted,
    case when Z.paidLate + Z.paidOnTime = 0 then 0 else round(1.0 * Z.tookLateDiscount / (Z.paidLate + Z.paidOnTime),2) end as tookLateDiscountPct,    
